    ...
    <header className="sticky top-0 z-30 border-b border-slate-200/60 bg-white/80 backdrop-blur-xl dark:border-slate-800 dark:bg-slate-900/80">
      <div className="mx-auto flex h-[60px] max-w-[1600px] items-center justify-between gap-3 px-3 sm:px-4">
        <div className="flex items-center gap-3">
          <Link to="/" className="flex items-center gap-2.5">
            ...
            <div className="hidden leading-tight sm:block">
              ...
              <div className="text-[10px] font-medium text-slate-500 dark:text-slate-400">{tr('subtitle', lang)}</div>
            </div>
            <span className="hidden rounded-full bg-amber-100 px-2 py-0.5 text-[10px] font-bold text-amber-800 lg:inline">SIH 26091</span>
          </Link>
          {loc.pathname!=='/' && <span className="hidden items-center gap-1.5 text-xs text-slate-400 sm:flex"><span className="h-3 w-px bg-slate-200"/> {breadcrumb}</span>}
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={() => setDark(v => !v)}
            className="rounded-xl border border-slate-200 bg-white px-2.5 py-2 text-xs font-semibold shadow-sm hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-800 dark:text-white"
            title={dark ? 'Switch to light' : 'Switch to dark'}
          >
            {dark ? '☀️' : '🌙'}
          </button>
          <select value={lang} onChange={e=>setLang(e.target.value as Language)} className="rounded-xl border border-slate-200 bg-white px-2.5 py-2 text-xs font-semibold shadow-sm dark:border-slate-700 dark:bg-slate-800 dark:text-white">
            <option value="en" className="dark:bg-slate-800">EN</option><option value="ta" className="dark:bg-slate-800">TA</option><option value="hi" className="dark:bg-slate-800">HI</option>
          </select>
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-slate-900/30 p-4 backdrop-blur-sm" onClick={onClose}>
      <div className="w-full max-w-lg overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl" onClick={e=>e.stopPropagation()}>
        <div className="flex items-center gap-3 border-b p-3">
          ...
          <input autoFocus value={q} onChange={e=>setQ(e.target.value)} placeholder="Jump to Dashboard, Finance, Schemes..." className="w-full bg-transparent text-sm outline-none placeholder:text-slate-400" />
        ...
        <div className="max-h-72 overflow-auto p-2">
          {items.map(i=>(
            <button key={i.to} onClick={()=>{ nav(i.to); onClose()}} className="flex w-full items-center justify-between rounded-xl px-3 py-2.5 text-left hover:bg-slate-50">
              <span className="text-sm font-semibold text-slate-800">{i.label}</span><span className="text-xs text-slate-400">{i.hint}</span>
            </button>
          ))}
          {items.length===0 && <div className="px-3 py-8 text-center text-sm text-slate-400">No results</div>}
        </div>
      </div>
    </div>
  )
