...
const GROUP_LABEL_KEY: Record<string, string> = { Plan: 'groupPlan', Discover: 'groupDiscover', Money: 'groupMoney', Output: 'groupOutput' }
  ...
  return (
    <aside className={`${collapsed ? 'w-[72px]' : 'w-[270px]'} hidden shrink-0 flex-col border-r border-slate-200/70 bg-white dark:border-slate-700 dark:bg-slate-900 lg:flex`}>
      <div className="space-y-6 p-4">
        {GROUPS.map(g => (
          <div key={g}>
            {!collapsed && <div className="mb-2 px-2 text-[10px] font-bold uppercase tracking-widest text-slate-500">{tr(GROUP_LABEL_KEY[g] as any, lang)}</div>}
