...
export function WeatherForecast({ lat, lon }: { lat: number; lon: number }) {
  ...
  if (loading) return <div className="rounded-xl bg-slate-50 p-3 text-xs text-slate-500">Loading 7-day forecast…</div>
  if (!days) return null
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-3">
      <div className="text-xs font-bold uppercase tracking-widest text-slate-500">7-day forecast (Open-Meteo)</div>
      ...
      <p className="mt-2 text-[10px] text-slate-500">For your exact pinned village — helps you plan stock before rain.</p>
    </div>
  )
}
