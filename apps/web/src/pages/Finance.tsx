...
export function Finance() {
  ...
  // EMI for prospective (deferred interest — no payment during moratorium, capitalised)
  const prospectiveEmi = (() => {
    ...
    try {
      ...
      const capitalised = r === 0 ? 0 : prospectiveLoan * (Math.pow(1 + r, mor) - 1)
      const bal = prospectiveLoan + capitalised
      if (r === 0) return bal / remaining
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2">
          {selectedSchemeCode && (
            <Badge color={isSchemeDriven ? 'green' : 'amber'}>{isSchemeDriven ? `${tr('scheme', lang)}: ${selectedSchemeName}` : `${tr('schemesSelected', lang)}: ${selectedSchemeCode}`}</Badge>
          )}
          {!needsRecalc && (
      ...
      {selectedSchemeCode && (
        <Card>
          <CardHeader title={tr('financingBeforeAfterTitle', lang) as any || "Financing — Before vs After Scheme"} subtitle={tr('financingBeforeAfterSub', lang) as any || "The selected scheme's actual terms (interest, tenure, moratorium, eligible loan) are applied to your funding gap"} />
          <div className="grid gap-4 md:grid-cols-2">
            <div className="rounded-xl border border-gray-200 bg-gray-50 p-4">
              <div className="text-xs font-bold uppercase tracking-widest text-gray-500">{tr('beforeSchemeGeneric', lang)}</div>
              <dl className="mt-2 space-y-1 text-sm text-gray-600">
                <div className="flex justify-between"><dt>{tr('projectRequirement', lang)}</dt><dd className="font-medium text-gray-900">₹{formatINR(fp.project_cost)}</dd></div>
                <div className="flex justify-between"><dt>{tr('yourCapital', lang)}</dt><dd className="font-medium text-gray-900">₹{formatINR(fp.capital_available)}</dd></div>
                <div className="flex justify-between"><dt>{tr('fundingGap', lang)}</dt><dd className="font-medium text-gray-900">₹{formatINR(fundingGap)}</dd></div>
                <div className="flex justify-between"><dt>{tr('assumptionLabel', lang)}</dt><dd className="text-xs text-gray-500">{tr('microFinanceDefault', lang)}</dd></div>
              </dl>
            </div>
            <div className="rounded-xl border border-brand-200 bg-brand-50 p-4">
              <div className="text-xs font-bold uppercase tracking-widest text-brand-700">{tr('afterScheme', lang)} {isSchemeDriven ? tr('applied', lang) : tr('notYetApplied', lang)}</div>
              {needsRecalc && selectedSchemeObj ? (
                <dl className="mt-2 space-y-1 text-sm">
                  <div className="flex justify-between"><dt className="text-brand-700">{tr('selectedSchemeLabel', lang)}</dt><dd className="font-bold text-brand-900">{selectedSchemeObj.name || selectedSchemeName || selectedSchemeCode}</dd></div>
                  {!isProspectiveEligible ? (
                    <div className="rounded bg-red-50 border border-red-200 px-2 py-2 text-xs text-red-700">
                      <strong>{selectedSchemeCode}</strong> {tr('notEligibleForCost', lang)} ₹{formatINR(fp.project_cost)} — {tr('outsideSchemeRange', lang)} {prospectiveMinCost != null ? `₹${formatINR(prospectiveMinCost)}` : '—'} – {prospectiveMaxCost != null ? `₹${formatINR(prospectiveMaxCost)}` : '∞'}. {tr('selectHigherCeiling', lang)}
                    </div>
                  ) : (
                    <>
                      <div className="flex justify-between"><dt>{tr('eligibleFinancing', lang)}</dt><dd className="font-medium text-gray-900">₹{formatINR(prospectiveLoan)} <span className="text-xs text-gray-500">(max ₹{formatINR(prospectiveMaxLoan!)})</span></dd></div>
                      <div className="flex justify-between"><dt>{tr('interestRate', lang)}</dt><dd className="font-medium text-gray-900">{prospectiveRate != null ? `${prospectiveRate}%` : tr('schemesUnknown', lang)}</dd></div>
                      <div className="flex justify-between"><dt>{tr('tenure', lang)}</dt><dd className="font-medium text-gray-900">{prospectiveTenure != null ? `${prospectiveTenure} ${tr('years', lang)}` : tr('schemesUnknown', lang)} {prospectiveMoratorium ? `· ${prospectiveMoratorium} ${tr('months', lang)} ${prospectiveMoratoriumMode ? ` (${prospectiveMoratoriumMode})` : ''}` : ''}</dd></div>
                      <div className="flex justify-between"><dt>{tr('monthlyEMI', lang)}</dt><dd className="font-bold text-brand-700">{prospectiveEmi ? `₹${formatINR(Math.round(prospectiveEmi))}` : '—'}</dd></div>
                      {prospectiveMaxLoan != null && fundingGap > prospectiveMaxLoan && <div className="rounded bg-amber-100 px-2 py-1 text-xs text-amber-800">{interpolate(tr('schemesAdditionalFunding', lang), { amount: formatINR(fundingGap - prospectiveMaxLoan) })}</div>}
                      {prospectiveMoratorium ? <div className="text-xs text-gray-600">{tr('moratorium', lang)}: {prospectiveMoratorium} {tr('months', lang)}{prospectiveMoratoriumMode ? ` — ${prospectiveMoratoriumMode}` : ''}</div> : null}
                      <div className="mt-2 text-[11px] text-amber-700">{interpolate(tr('clickApplyToRecalc', lang), { code: selectedSchemeCode })}</div>
                    </>
                  )}
                </dl>
              ) : (
                <dl className="mt-2 space-y-1 text-sm">
                  <div className="flex justify-between"><dt className="text-brand-700">{tr('selectedSchemeLabel', lang)}</dt><dd className="font-bold text-brand-900">{selectedSchemeName || selectedSchemeCode}</dd></div>
                  <div className="flex justify-between"><dt>{tr('eligibleFinancing', lang)}</dt><dd className="font-medium text-gray-900">₹{formatINR(fp.loan_amount)} {fp.max_loan != null && <span className="text-xs text-gray-500">(max ₹{formatINR(fp.max_loan)})</span>}</dd></div>
                  <div className="flex justify-between"><dt>{tr('interestRate', lang)}</dt><dd className="font-medium text-gray-900">{fp.interest_rate != null ? `${fp.interest_rate}%` : tr('schemesUnknown', lang)} {fp.is_assumed && <span className="text-[10px] text-amber-600">({tr('assumedLabel', lang)})</span>}</dd></div>
                  <div className="flex justify-between"><dt>{tr('tenure', lang)}</dt><dd className="font-medium text-gray-900">{fp.tenure_years != null ? `${fp.tenure_years} ${tr('years', lang)}` : tr('schemesUnknown', lang)} {fp.moratorium_months ? `· ${fp.moratorium_months} ${tr('months', lang)} (${fp.moratorium_mode})` : ''}</dd></div>
                  <div className="flex justify-between"><dt>{tr('monthlyEMI', lang)}</dt><dd className="font-bold text-brand-700">₹{formatINR((result as any)?.unified_financial?.emi || (result as any)?.repayment?.monthly_emi || 0)}</dd></div>
                  {fp.shortfall > 0 && <div className="rounded bg-amber-100 px-2 py-1 text-xs text-amber-800">{interpolate(tr('schemesAdditionalFunding', lang), { amount: formatINR(fp.shortfall) })}</div>}
                  {fp.moratorium_months > 0 && <div className="text-xs text-gray-600">{tr('moratorium', lang)}: {fp.moratorium_months} {tr('months', lang)} — {fp.moratorium_mode}</div>}
                </dl>
              )}
              {needsRecalc && selectedSchemeObj && !isProspectiveEligible && (
                <div className="mt-3 rounded-lg bg-red-50 border border-red-200 p-3 text-xs text-red-700">
                  <strong>{selectedSchemeCode}</strong> {tr('notEligibleForCost', lang)} ₹{formatINR(fp.project_cost)} — {fp.scheme_reason || tr('exceedsSchemeLimits', lang)}. {tr('tryTermLoan', lang)}
                </div>
              )}
              {needsRecalc && (
                <button onClick={handleApplyScheme} disabled={recalcLoading} className="mt-3 w-full rounded-lg bg-brand-600 px-4 py-2 text-xs font-bold text-white hover:bg-brand-700 disabled:opacity-50">
                  {recalcLoading ? tr('recalculating', lang) : interpolate(tr('applyToFinanceRecalc', lang), { code: selectedSchemeCode })}
                </button>
              )}
              {!isSchemeDriven && !needsRecalc && <div className="mt-2 text-xs text-gray-500">{tr('financeGenericTerms', lang)}</div>}
            </div>
          </div>
          <p className="mt-3 text-xs text-gray-500">{interpolate(tr('financingFormula', lang), { gap: `₹${formatINR(fundingGap)}`, max: `₹${needsRecalc && prospectiveMaxLoan != null ? formatINR(prospectiveMaxLoan) : fp.max_loan != null ? formatINR(fp.max_loan) : '∞'}` })}</p>
        </Card>
      )}
      ...
      {costBreakdown && (
        <Card>
          ...
          <div className="space-y-4">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <CostSection title={tr('shopEquipmentTitle', lang)} items={costBreakdown.capital_expenditure} />
              <CostSection title={tr('workingCapitalTitle', lang)} items={costBreakdown.working_capital} />
              <CostSection title={tr('infrastructureTitle', lang)} items={costBreakdown.infrastructure} />
              <CostSection title={tr('licensingTitle', lang)} items={costBreakdown.licensing_compliance} />
            </div>
            <div className="flex items-center justify-between border-t pt-2 text-sm">
              <span className="text-gray-500">{tr('contingencyLabel', lang)} ({costBreakdown.contingency_pct}%)</span>
              <span className="font-medium">₹{formatINR(costBreakdown.contingency_amount)}</span>
            </div>
            <div className="flex items-center justify-between border-t pt-2 text-base font-bold">
              <span>{tr('totalProjectCostLabel', lang)}</span>
      ...
      {!selectedSchemeCode && fp.scheme_code == null && (
        <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">
          <strong>{interpolate(tr('noSchemeSelectedCost', lang), { cost: `₹${formatINR(fp.project_cost)}` })}</strong> {tr('pleaseSelectScheme', lang)} <a href="/schemes" className="font-bold underline">{tr('schemesPageLink', lang)}</a> {tr('showingConceptEstimates', lang)} — {interpolate(tr('fundingGapWillBeFinanced', lang), { gap: `₹${formatINR(fundingGap)}` })}
        </div>
      )}
      <div className="grid gap-4 md:grid-cols-4">
        <StatCard label={tr('projectCost', lang)} value={`₹${formatINR(fp.project_cost)}`} sub={tr('projectCostSub', lang)} />
        <StatCard label={tr('loanAmount', lang)} value={fp.scheme_code || selectedSchemeCode ? `₹${formatINR(fp.loan_amount)}` : '—'} sub={fp.scheme_code || selectedSchemeCode ? `${tr('ownContribution', lang)} ₹${formatINR(fp.own_contribution ?? 0)}` : interpolate(tr('fundingGapSelectScheme', lang), { gap: `₹${formatINR(fundingGap)}` })} />
      ...
      <Card>
        <CardHeader title={tr('financingPlanOkayTitle', lang)} subtitle={tr('financingPlanOkaySub', lang)} />
        <div className="flex flex-wrap gap-3">
          <button onClick={() => { setFinanceConfirmed(true); navigate('/simulator') }} className="rounded-xl bg-brand-600 px-6 py-2.5 text-sm font-bold text-white hover:bg-brand-700">{tr('okayContinue', lang)}</button>
          <button onClick={() => navigate('/schemes')} className="rounded-xl border border-slate-200 bg-white px-6 py-2.5 text-sm font-bold text-slate-700 hover:bg-slate-50">{tr('backLabel', lang)}</button>
        </div>
        {financeConfirmed && <p className="mt-2 text-xs text-green-600">{tr('financingConfirmedMsg', lang)}</p>}
      </Card>
          ...
          {(() => {
            ...
            return (
              <>
                ...
                {showProspective && <p className="mt-2 text-[11px] text-amber-600">{tr('showingSelectedSchemeNotApplied', lang)}</p>}
                ...
                {showProspective && selectedSchemeObj && (
                  <p className="mt-3 rounded-lg bg-amber-50 p-2 text-xs text-amber-800">{interpolate(tr('schemeWillBeApplied', lang), { gap: `₹${formatINR(fundingGap)}` })}</p>
                )}
              </>
            )
          })()}
            ...
            <div className="space-y-3 text-sm text-gray-600">
                ...
                if (!fp.scheme_code && !selectedSchemeCode) {
                  return (
                    <p className="rounded-lg bg-amber-50 p-3 text-amber-800">
                      {interpolate(tr('withCapitalProjectCost', lang), { capital: `₹${formatINR(fp.capital_available)}`, cost: `₹${formatINR(fp.project_cost)}`, own: `₹${formatINR(fp.own_contribution ?? 0)}`, req: `₹${formatINR(fp.required_financing ?? fundingGap)}` })} <br />{tr('noSchemeSelectedConcept', lang)} — {tr('pleaseSelectScheme', lang)} <a href="/schemes" className="font-bold underline">{tr('schemesPageLink', lang)}</a> {tr('toSeeInterestTenure', lang)}
                    </p>
                  )
                }
                if (needsRecalc && selectedSchemeObj) {
                  const morText = prospectiveMoratorium ? ` — ${interpolate(tr('noPaymentDuringMoratorium', lang), { n: prospectiveMoratorium })}` : ''
                  return (
                    <p>
                      ...
                      {morText}. <span className="text-amber-700">{tr('clickApplyToRecalcShort', lang)}</span>
                    </p>
                  )
                }
                return (
                  <p>
                    ...
                    {moratorium > 0 ? ` — ${interpolate(tr('noPaymentDuringMoratorium', lang), { n: moratorium })}` : ''}.
                  </p>
                )
              })()}
              ...
              <p>
                {(() => {
                  ...
                  if (!fp.scheme_code && !selectedSchemeCode) return tr('estimatedInstalmentFallback', lang)
                  if (needsRecalc) return interpolate(tr('estimatedInstalmentWithCode', lang), { code: selectedSchemeCode, emi: prospectiveEmi ? `₹${formatINR(Math.round(prospectiveEmi))}` : '—' })
              ...
              {!(needsRecalc && isProspectiveEligible) && (
      ...
      <dl className="divide-y divide-gray-100">
        {entries.map(([k, v]) => (
          <div key={k} className="flex items-center justify-between gap-3 py-1.5 text-sm">
            <dt className="min-w-0 flex-1 break-words text-gray-500">{translateCostItemFinance(k, lang)}</dt>
...
function translateCostItemFinance(name: string, lang: Language): string {
  if (!name || lang === 'en') return name
  const map: Record<string, keyof typeof import('../lib/i18n').dict> = {
    'Milking machine (electric)': 'costMilkingMachine',
    'Milk cans, coolers, utensils': 'costMilkCans',
    'Initial animal purchase (5 cows)': 'costAnimalPurchase',
    'Animal shed construction': 'costAnimalShed',
    'Bulk milk cooler': 'costBulkCooler',
    'Shed with concrete floor & roof': 'costShedFloor',
    'FSSAI state license': 'costFssai',
    'Shop & Establishment license': 'costShopLicense',
    'Trade license': 'costTradeLicense',
    'Fodder & feed (1 month)': 'costFodder',
    'Labour (2 helpers)': 'costLabour',
    'Veterinary care reserve (1 month)': 'costVetReserve',
    'Transportation (1 mini-van share)': 'costTransport',
    'Electricity & misc': 'costElectricity',
    'Bore-well / water supply': 'costBorewell',
    'Manure pit': 'costManurePit',
    // micro dairy fallbacks
    'Milking equipment (hand/manual)': 'costMilkingHand',
    'Milk cans & utensils': 'costMilkCansSmall',
    'Initial animal purchase (2 cows/buffaloes)': 'costAnimalMicro',
    'Animal shed basics (tarpaulin/ply)': 'costShedMicro',
    'Cooling/refrigeration (small fridge)': 'costCoolingMicro',
    'Transportation (milk collection/delivery)': 'costTransportMicro',
    'Labour (1 helper)': 'costLabourMicro',
    'Shed flooring & fencing': 'costShedFlooringMicro',
    'Water supply connection': 'costWaterMicro',
    'FSSAI basic registration': 'costFssaiMicro',
    'Udyam/MSME registration': 'costUdyamMicro',
    // medium dairy
    'Automated milking system': 'costAutoMilkingMedium',
    'Bulk milk cooler (500L)': 'costBulk500Medium',
    'Initial animal purchase (10 cows)': 'costAnimal10Medium',
    'Shed with roofing & drainage': 'costShedDrainMedium',
    'Delivery vehicle': 'costDeliveryMedium',
  }
  const key = map[name]
