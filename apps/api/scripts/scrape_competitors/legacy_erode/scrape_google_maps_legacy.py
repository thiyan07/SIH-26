...
def main(argv=None) -> int:
    ...
    ap = argparse.ArgumentParser(description="LEGACY Erode scraper — use scripts.discovery.run_discovery for production")
    ...
    ap.add_argument("--allow-legacy", action="store_true", help="Acknowledge this is legacy Erode scraper")
    args = ap.parse_args(argv)

    if not args.allow_legacy:
        ap.error(
            "This is the legacy Erode scraper (TOWNS/KEY_VILLAGES). "
            "Use scripts.discovery.run_discovery for the production Tamil Nadu discovery engine. "
            "Pass --allow-legacy to run the legacy scraper anyway for historical reproducibility."
        )
